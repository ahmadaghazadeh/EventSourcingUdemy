...
namespace Post.Query.Infrastructure.Dispatchers
{
	...
	{
		private readonly Dictionary<Type, Func<BaseQuery, Task<List<PostEntity>>>> _handers = new();
		...
		public void RegisterHandler<TQuery>(Func<TQuery, Task<List<PostEntity>>> handler) where TQuery : BaseQuery
		{
			...
			_handers.Add(typeof(TQuery), x => handler((TQuery)x));
		}

		public async Task<List<PostEntity>> SendAsync(BaseQuery query)
		{
			if (_handers.TryGetValue(query.GetType(), out Func<BaseQuery, Task<List<PostEntity>>> handler))
			{
